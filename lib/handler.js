...
const regex = { // Strings for sending and comparing messages to regex
	...
	wifi: { text: 'Vamos a buscar zonas wifi cercanas. ', table: 'NearBot_Madrid_Wifi', type: 'wifi', filterDB: false },
}
...
bot.command('novedades', (msg, reply) => {
	stats.duration.sentRes = new Date().getTime() - stats.date
	reply.markdown('*Versión 2.0.0*\n\n- (!) Ahora, los resultados de BiciMAD incluyen el estado de la estación en tiempo real\n- (!) Se ha añadido un filtro para los sitios que estén cerrados en el momento de la consulta\n- (!) Nueva opción para avisar de posibles errores en el resultado\n- (!) Se ha añadido la opción de búsqueda de zonas wifi públicas\n- Reestructuración completa del código fuente (>2x velocidad de respuesta)\n- Se ha reescrito el sistema de búsqueda\n- Actualización de librerías\n- Corrección de errores').then((err, res) => postMessage(err, res, stats))
...
// Place request
bot.command('metro', 'cercanias', 'metroligero', 'transporte', 'fuente', 'bici', 'aseo', 'carrefour', 'mercadona', 'supermercado', 'wifi', (msg, reply) => {
	stats.duration.sentRes = new Date().getTime() - stats.date
...
bot.location((msg, reply) => {
	...
	if (msg.reply) { // If the location is a reply to a message (serverless tricks)
		...
		reply.inlineKeyboard([
			...
			[{ text: 'Wifi', callback_data: JSON.stringify({ t: 'wifi', c: coordinates }) }]
	...
	if (data.t) {
		...
		case 'menu':
			...
			reply.inlineKeyboard([
				...
				[{ text: 'Wifi', callback_data: JSON.stringify({ t: 'wifi', c: data.c }) }]
