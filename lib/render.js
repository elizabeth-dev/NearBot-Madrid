...
async function processResult(type, coordinates, reply, result) {
	...
	let bikeStatus = ''
	if (type === 'bike') {
		bikeStatus = await getBikeStatus(result.id)
	}

	let resultCoordinates = result.coords.toString()
	reply.keyboard().text('Aquí tienes: ')
	reply.inlineKeyboard(replyKeyboard(resultCoordinates)).photo(getMap(coordinates, resultCoordinates), emoji + name + distance + metroName + commuterName + tramName + bikeStatus + '\n' + street + phone + timetable + metroLines + commuterLines + tramLines, 'Markdown')
}
