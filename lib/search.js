...
function filterTimetable(results) {
	...
	let currentTime = parseInt(currentDate.getUTCHours().toString() + currentDate.getUTCMinutes().toString())
...
async function searchQuery(coordinates, config, stats) {
	let params = {
	...
	stats.duration.startDB = new Date().getTime() - stats.date
	let data = await ddb.scan(params).promise().catch((err) => {
		console.error(err)
	})
	stats.duration.finishDB = new Date().getTime() - stats.date

	let results = data.Items
	if (results[0].timetable) {
		results = filterTimetable(results)
	}
	stats.duration.startSearch = new Date().getTime() - stats.date
	let result = getResult(coordinates, results)
	stats.duration.finishSearch = new Date().getTime() - stats.date
	return [result, stats]
}
