// Script to upload listaFuentes.json
...
const config = new ddbGeo.GeoDataManagerConfiguration(ddb, 'NearBot_Madrid_Fuente')
...
ddb.createTable(createTable).promise()
	...
	// Load data
	.then(function () {
		console.log('Loading from listaFuentes.json')
		const data = require('./databases/listaFuentes.json')
		const putPointsInputs = data.map(function (fuente) {
			return {
				RangeKeyValue: { S: fuente.NUMEROFUENTE.toString() },
				GeoPoint: {
					latitude: fuente.LAT,
				...
				PutItemInput: {
					Item: {
						codigoFuente: AWS.DynamoDB.Converter.input(fuente.CODIGOFUENTE),
						denominacion: AWS.DynamoDB.Converter.input(fuente.DENOMINACION),
						calle: AWS.DynamoDB.Converter.input(fuente.CALLE)
					}
				}
			}
		})
